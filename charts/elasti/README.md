...
| `global.allowedNamespaces`          | namespaces KubeElasti is confined to. Empty (default) installs cluster-scoped RBAC and watches all namespaces. When set, installs namespace-scoped Role/RoleBinding in each listed namespace (the release namespace is always included) and confines the operator and resolver to those namespaces. | `[]`            |
